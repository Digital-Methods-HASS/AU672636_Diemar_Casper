...
leaflet() %>%
  ...
  addProviderTiles("Esri.WorldImagery",       # add custom Esri World Physical map tiles
                   options = providerTileOptions(opacity=0.5)) %>%     # make the Esri tile transparent
  setView(lng = 9.0586993, lat = 56.1488816 , zoom = 7)
...
# Answer
# The cluster option made the map have a cleaner presentation. But we miss the different layers to look at. 

# Task 5: Find out how to display notes and classifications in the map.

# Answer
# To make more popups i add the following code popup = paste("<strong>Type: </strong>",places$Type,"<br><strong>descripition : </strong>",places$Description), to the previous one
